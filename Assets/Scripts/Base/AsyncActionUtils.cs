...
using EaseFunc = System.Func<float, float>;
...
public static partial class AsyncActionUtils
{
    ...
    public static IEnumerator MoveCoroutine(MonoBehaviour agent, Transform transform, Transform from, Transform to, float time, VectorTweener tween, float delayBeforeStart, System.Action onComplete)
    {
        ...
        yield return ProgressiveActionCoroutine(time, null, (float progress) =>
        {
            if (!(agent == null || transform == null || from == null || to == null))
                transform.position = tween(from.position, to.position, progress);
        }, onComplete);
...
public static partial class AsyncActionUtils
{
    ...
    public static Coroutine DoPath(MonoBehaviour agent, Transform transform, Vector3[] path, float time, EaseFunc easeFunc, float delayBeforeStart = 0f, System.Action onComplete = null)
    {
        transform.position = path[0];
        return agent.StartCoroutine(DoPath(transform, path, time, easeFunc, delayBeforeStart, onComplete));
    }

    public static IEnumerator DoPath(Transform transform, Vector3[] path, float time, EaseFunc easeFunc, float delayBeforeStart, System.Action onComplete)
    {
        yield return new WaitForSeconds(delayBeforeStart);

        if (transform == null)
        {
            Debug.LogWarning("MoveCoroutine failure.transform: " + transform);
            yield break;
        }
        ...
        yield return ProgressiveActionCoroutine(time, null, (float progress) =>
        {
            ...
            float t = easeFunc(progress);  //缓动函数easeFunc 返回 0-1

            if (t > 1f || t < 0f)
            {
                Debug.LogWarning("easeFunc result mast between:  [0,1] ");
                return;
            }

            float count = t * (path.Length - 1);
            float integerPart = (float)Math.Floor(count); // 获取整数部分
            float decimalPart = count - integerPart;     // 获取小数部分

            int index = (int)integerPart;
            //Debug.Log($"index = {index} Pro = {progress} t = {t} Length = {path.Length}  integerPart = {integerPart} decimalPart = {decimalPart}  ");
            if (decimalPart > 0f && index < path.Length - 1)
            {
                VectorTweener tween = TweenUtils.VectorTweenLinear;
                transform.position = tween(path[index], path[index + 1], decimalPart);
            }
            else
            {
