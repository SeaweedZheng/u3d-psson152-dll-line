...
{
    public class PopupConsoleSetServerConnect : PageMachineButtonBase
    {
        public TextMeshProUGUI tmpTitle, tmpTip1, tmpTip2;

        public Button btnClose, btnConfirm;

        public CompInputController compInputIP1;
        public CompInputController compInputIP2;
        public CompInputController compInputIP3;
        public CompInputController compInputIP4;
        ...
        void OnClickConfirm()
        {
            ...
            int ip1 = string.IsNullOrEmpty(compInputIP1.value) ? -1:int.Parse(compInputIP1.value);
            int ip2 = string.IsNullOrEmpty(compInputIP2.value) ? -1 : int.Parse(compInputIP2.value);
            int ip3 = string.IsNullOrEmpty(compInputIP3.value) ? -1 : int.Parse(compInputIP3.value);
            int ip4 = string.IsNullOrEmpty(compInputIP4.value) ? -1 : int.Parse(compInputIP4.value);

            int port = string.IsNullOrEmpty(compInputPort.value) ? -1 : int.Parse(compInputPort.value);
            ...
            {
                string addr = $"{ip1}.{ip2}.{ip3}.{ip4}:{port}";
