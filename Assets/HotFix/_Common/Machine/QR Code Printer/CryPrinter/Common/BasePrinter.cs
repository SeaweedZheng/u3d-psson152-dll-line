...
namespace CryPrinter
{
    ...
    public abstract class BasePrinter : IPrinter
    {
        ...
        {
            Debug.Log("Restoring all default options and configurable . . .");
            ...
            _stream = new MemoryStream();
            _docBuffer = new BinaryWriter(_stream);
            Debug.Log("Success Clear Doc Buffer");
        ...
        {
            Debug.Log("Setting font scalars . . .");
        ...
        {
            Debug.Log("Removing font effects . . .");
            ...
            foreach (var flag in effect.GetFlags())
            {
                ...
                if (DisableCommands.ContainsKey(flag))
                {
                    ...
                    if (cmd.Length > 0)
                    {
                        ...
                        if (ret != ReturnCode.Success)
                        {
                            Debug.LogError("Failed to remove all effects . . .");
                            result = ReturnCode.ExecutionFailure;
                        }
                    }
                }
        ...
        {
            Debug.Log("Clearing all font effects . . .");
            ...
            foreach (var cmd in DisableCommands.Values)
            {
                if (cmd.Length > 0)
                {
                    ...
                    if (ret != ReturnCode.Success)
                    {
                        Debug.LogError("Failed to clear all effects . . .");
                        result = ReturnCode.ExecutionFailure;
                    }
                }
            }
        ...
        {
            Debug.Log("Printing the following ASCII string: " + str);
        ...
        {
            Debug.Log("Marking ticket as complete and presenting");
            ...
            if (payload.Length == 0)
            {
                Debug.LogWarning("Warning: payload is empty");
            ...
            catch(Exception e)
            {
                ...
                Debug.LogError("FormFeed Close connection");
                Connection.Close();
                return ReturnCode.ExecutionFailure;
            }
            finally
        ...
        public virtual ReturnCode ClearDocBuffer()
        {
            _docBuffer.Dispose();
            _stream = new MemoryStream();
            _docBuffer = new BinaryWriter(_stream);
        ...
        {
            Debug.LogWarning($"Dispose Close connection: {disposing}");
            Connection.Close();
            Connection?.Dispose();
            _docBuffer.Dispose();
        }
